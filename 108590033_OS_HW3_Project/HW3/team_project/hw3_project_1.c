...
int Beavailable[NUMBER_OF_RESOURCES];
int Bemaximum[NUMBER_OF_CUSTOMERS][NUMBER_OF_RESOURCES];
int Beallocation[NUMBER_OF_CUSTOMERS][NUMBER_OF_RESOURCES];
int Beneed[NUMBER_OF_CUSTOMERS][NUMBER_OF_RESOURCES];
...
int bankerAlgorithm(int customer_num,int request[]){
    ...
    for(int i=0;i<NUMBER_OF_RESOURCES;i++){ // copy the matrices
        Beavailable[i]=available[i];
        for(int j=0;j<NUMBER_OF_CUSTOMERS;j++){
            Beallocation[j][i]=allocation[j][i];
            
            Bemaximum[j][i]=maximum[j][i];
        ...
        for(int i=0;i<NUMBER_OF_RESOURCES;i++){ // pretend to give the resource to the thread
            Beavailable[i]=Beavailable[i]-request[i];
            Beallocation[customer_num][i]=Beallocation[customer_num][i]+request[i];
            Beneed[customer_num][i]=Beneed[customer_num][i]-request[i];
        }
...
//*************************safety Algorithm***************************
    ...
    while(1){
        ...
        for(int i=0;i<NUMBER_OF_CUSTOMERS;i++){ // to find a thread that its need is less than or equal to available.
            int nLessThanA=1;
            for(int j=0; j<NUMBER_OF_RESOURCES;j++){
                if(Beneed[i][j]>Beavailable[j] || finish[i]==1){
                    nLessThanA=0;
                    break;
                }
        ...
        if(I!=-1){
            ...
            for(int k=0;k<NUMBER_OF_RESOURCES;k++){  // pretend to give the reaource to thread
                Beavailable[k]=Beavailable[k]+Beallocation[I][k];
            }
